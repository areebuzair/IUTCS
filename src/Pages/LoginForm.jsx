...
const LoginForm = () => {
  ...
  const { setIsAuthenticated } = useAuth(); // Get isAuthenticated and setIsAuthenticated from AuthContext
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post('http://localhost:5050/iut-cse/admin/login', formData)
      .then((response) => {
        console.log(response);
        console.log('OK');
        setIsAuthenticated(true); // Set isAuthenticated to true after successful login
        navigate('/blogs'); // Redirect to the protected route
      })
      .catch((error) => {
        console.error('Error saving information:', error);
        if (error.response) setErrorMessage('Incorrect username or password');
        else setErrorMessage('Could not connect to the server');
      });
  };
